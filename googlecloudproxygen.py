...
class GoogleCloudProxyGen:
    ...
    def create_vm(self, project, zone, startup_script):
    ...
    def delete_vm(self, project, zone, server_name):
        return self.compute_client.instances().delete(project=project, zone=zone, instance=server_name).execute()
    
    def get_vm_ip_address(self, project, zone, server_name):
        while(True):
            try:
                server_ip = self.compute_client.instances().get(project=project, zone=zone, instance=server_name).execute()['networkInterfaces'][0]['accessConfigs'][0]['natIP']
                return server_ip
    ...
    def create_proxies(self, project, region, proxy_count, proxy_username, proxy_password):
        ...
        for x in range(proxy_count):
            server_name = self.create_vm(project, region, startup_script)
            server_ip = self.get_vm_ip_address(project, region, server_name)
            servers.append(proxymodels.GoogleCloudServer(project, region, server_name, server_ip, proxy_username, proxy_password, 80))
